<template>
  <div class="container mx-auto">
    <div class="mobile-screen relative w-full p-6">
      <ValidationObserver ref="form">
        <div class="mobile-content">
          <validation-provider v-slot="{ errors }" name="Jawaban" rules="required">
            <p>
              Apakah Anda pernah mendapatkan undangan mengikuti Pikobar Pendaftaran Tes COVID-19 dari PIKOBAR atau instansi lainnya?
            </p>
      ...
      <form-actions class="mobile-actions absolute inset-x-0 bottom-0" back-link="/survey/hello" @next="nextStep" />
    </div>
  </div>
</template>
    ...
    async nextStep () {
      ...
      if (valid) {
        if (this.testInvited === 1) {
          this.$router.push('/survey/q_attend')
        } else {
